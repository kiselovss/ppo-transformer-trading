"""
PPO Transformer Tirdzniecības Aģents
...
RISK CONTROL MODE REŽĪMS
─────────────────────────
Šis eksperiments paplašina Realistic Mode ar aktīvu drawdown kontroli.
Visi pamatparametri paliek identiski galvenajai versijai, bet tiek aktivizēts
viens atslēgts mehānisms:

  - Drawdown soda koeficients (DD_COEF = 0.2)
      pamatversijā DD_COEF = 0.0 (atslēgts)
      šeit aktivizēts: sods = dd * DD_COEF katru soli pozīcijā
      pie 5% krituma  - papildu -0.010 reward katru soli
      pie 10% krituma - papildu -0.020 reward katru soli

Visi pārējie parametri paliek nemainīgi:
  - OPEN_PENALTY       = 0.000898  (identisks pamatversijai)
  - MIN_TRADE_BARS     = 72        (identisks pamatversijai)
  - SHORT_TRADE_PEN    = 0.000898  (identisks pamatversijai)
  - GRACE_PERIOD_STEPS = 12        (identisks pamatversijai)

Mērķis: aģents iemācās ne tikai meklēt peļņu, bet arī izvairīties
no bīstamiem periodiem un agri slēgt zaudējošas pozīcijas,
pirms kritums kļūst pārāk liels.

IZMANTOŠANA
───────────
Salīdzinu bektesta rezultātus ar Realistic Mode (pamatversiju):
  Ja max drawdown samazinās, tad DD sods darbojas kā paredzēts
  Ja Calmar koeficients pieaug, tad riska/peļņas attiecība uzlabojusies
  Ja darījumu skaits krasi krīt, tad DD_COEF iespējams pārāk agresīvs (mēģināt 0.1)
...
────────────────────────────────────────────────────────────────────────────────
  GPU (ieteicams):
    ...
    GPU ieteicamās OPEN_PENALTY vērtības:
        0.0064  - pamatrežīms     (20-100 darījumi testā)
        0.007   - konservatīvais  (5-20 darījumi)
        0.004   - aktīvais        (100-300 darījumi)

  CPU:
    Testēts uz AMD Ryzen 3 1200 Quad-Core, apmācība bez AMP,
      aptuveni 30-60 min uz 75 000 soļiem.
    Jutīgums pret OPEN_PENALTY ir ievērojami augstāks nekā uz GPU.
      CPU ieteicamās OPEN_PENALTY vērtības:
        0.000898 - pamatrežīms     (20-100 darījumi testā)
        0.004    - konservatīvais  (5-20 darījumi)
        0.0008   - aktīvais        (200-1000 darījumi)
        0.0001   - eksperimentāls  (iespējama pārmērīga tirdzniecība >1000 darījumi)
...
# agrīnās versijas izmantoja 200_000+ soļus ar smagākiem modeļiem.
...
# Eksperimenta bloks:
# Visi reward parametri ir koncentrēti šeit.
#
# OPEN_PENALTY ir galvenais parametrs pamatversijas apmācībā - nosaka tirdzniecības biežumu.
# OPEN_PENALTY vērtības tika atlasītas empīriski.
# Optimālās OPEN_PENALTY vērtības būtiski atšķiras uz GPU un CPU:
# GPU apmācība ir stabilāka un pakļauta pārmērīgai tirdzniecībai pie zemām vērtībām,
# tāpēc GPU nepieciešami augstāki sodi (skatīt faila sākumu).
EXPERIMENT = {
    "name"               : "Risk Control Mode",
    "SEED"               : SEED,
    "TOTAL_STEPS"        : TOTAL_STEPS,

    "HOLD_PENALTY_NO_POS": -0.003,
    "INVALID_ACTION_PEN" : -0.05,
    "REWARD_CLIP"        : 0.3,

    # Drawdown soda koeficients — galvenā atšķirība no pamatversijas.
    # Pamatversijā DD_COEF = 0.0 (atslēgts). Šeit aktivizēts ar vērtību 0.2.
    # Sods tiek aprēķināts kā: dd * DD_COEF, kur dd = (peak_eq - floating_eq) / peak_eq.
    # Pie DD_COEF = 0.2 un 10% krituma aģents saņem papildu -0.02 katru soli pozīcijā.
    # Vērtība 0.2 izvēlēta kā kompromiss: pietiekami jūtama, bet neaizēno PnL signālu.
    "DD_COEF"            : 0.2,

    # Sveces skaits pēc pozīcijas slēgšanas, kuru laikā HOLD netiek sodīts.
    "GRACE_PERIOD_STEPS" : 12,

    # Sods par darījuma atvēršanu — identisks pamatversijai.
    "OPEN_PENALTY"       : 0.000898,

    # Minimālais sveces skaits pozīcijā — identisks pamatversijai.
    "MIN_TRADE_BARS"     : 72,

    # Sods par pārāk īsu pozīciju — identisks pamatversijai.
    "SHORT_TRADE_PEN"    : 0.000898,
}
...
# Tas novērš datu noplūdi (agrīnās koda versijās scaler tika apmācīts uz visas datu kopas, kas deva negodīgu priekšrocību).
...
# Agrīnās versijās nederīgas darbības vienkārši tika ignorētas, kas maldināja aģentu apmācības laikā.
# ─────────────────────────────────────────────────────────────────────────────
...
# 4. solis - Atlīdzības funkcija
#
# Galvenie reward shaping mehānismi:
#   GRACE_PERIOD - buferis pēc slēgšanas, izslēdzot sodu par domāšanu
#   OPEN_PENALTY - galvenais svira tirdzniecības biežuma kontrolei
#   SHORT_TRADE_PEN - anti-skalping sods par ātru slēgšanu
...
def compute_reward(action, position, floating_eq, prev_balance,
    ...
    """
    Atlīdzības politika:
    1. Nederīga darbība                 = INVALID_ACTION_PEN
    2. HOLD bez pozīcijas labvēlības periodā = 0.0  (aģents domā, skaitām neitrāli)
    3. HOLD bez pozīcijas ārpus labvēlības   = HOLD_PENALTY_NO_POS
    4. Pozīcijas atvēršana              = log_ret - OPEN_PENALTY
    5. Pozīcijas turēšana               = log_ret (bez soda)
    6. Slēgšana ātrāk par MIN_TRADE_BARS = reward - SHORT_TRADE_PEN
    7. Visas vērtības tiek apgrieztas ±REWARD_CLIP

    bars_in_trade - sveces skaits pašreizējā pozīcijā.
                    Izmanto anti-skalping sodam (p.6).
    """
    ...
    if action == 0 and position == 0:
        if steps_since_close < cfg["GRACE_PERIOD_STEPS"]:
            return 0.0
        return cfg["HOLD_PENALTY_NO_POS"]

    log_ret = math.log(floating_eq / (prev_balance + 1e-9) + 1e-9)
    dd      = (peak_eq - floating_eq) / (peak_eq + 1e-9)
    reward  = log_ret - dd * cfg["DD_COEF"]

    # Sods par atvēršanu - galvenais instruments darījumu biežuma kontrolei.
    # Tika atlasīts empīriski, kā aprakstīts iepriekš, un optimālās vērtības atšķiras GPU un CPU (sīkāk faila sākumā un EXPERIMENT blokā).
    if opened_position:
        reward -= cfg["OPEN_PENALTY"]

    # Anti-skalping sods: pozīcijas slēgšana ātrāk par MIN_TRADE_BARS svecēm.
    # Darbojas tikai ja pozīcija bija atvērta (bars_in_trade > 0).
    if action == 3 and bars_in_trade < cfg["MIN_TRADE_BARS"]:
        reward -= cfg["SHORT_TRADE_PEN"]

    return float(np.clip(reward, -cfg["REWARD_CLIP"], cfg["REWARD_CLIP"]))


# 5. solis - Tirdzniecības vide (TradingEnv)
# Viena tirdzniecības aģenta simulators. Savienojas ar PPO caur standarta saskarni reset() / step().
#
# Galvenie dizaina lēmumi:
# Lēmums tiek pieņemts pēc pašreizējās sveces stāvokļa, bet izpilde vienmēr pēc nākamās sveces OPEN cenas, kas novērš nākotnes datu noplūdi.
...
class TradingEnv:
    ...
    def reset(self, fixed_start=None):
        ...
        Apmācības laikā (fixed_start=None) sākuma pozīcija tiek izvēlēta nejauši pieļaujamā diapazonā -
        tas dod daudzveidību sākuma apstākļos starp paralēlajām VecEnv vidēm.
        ...
        self._last_trade      = None
        # steps_since_close sākas ar lielu skaitli, lai labvēlības periods neuzkrātos velti pirms pirmā ieejas tirgū.
        self.steps_since_close = 999
        self.bars_in_trade     = 0    # sveces skaitītājs pašreizējā pozīcijā
        return self._obs()

    def _obs(self):
        """
        Veido novērojumu: SEQ_LEN sveces logu ar tirgus pazīmēm un divas aģenta stāvokļa kolonnas (position, unrealized_pnl).
        Šāda savienošana ļauj modelim vienlaicīgi redzēt tirgus vēsturi un savu pozīciju bez atsevišķa stāvokļa iegulšanas.
        """
    ...
    def step(self, action):
        ...
        Šāda secība garantē nākotnes datu noplūdes neesamību: lēmums pieņemts uz i-1 sveces datiem, izpildīts pēc i sveces atvēršanas cenas.
        """
        self.i += 1
        ...
        # _execute_close() atiestatīs steps_since_close uz 0 slēdzot.
        # _execute_open() atiestatīs bars_in_trade uz 0 atverot.
        if self.position == 0:
            self.steps_since_close += 1
        else:
            self.bars_in_trade += 1

        # Peldošais kapitāls - pozīcijas pašreizējā vērtība pēc close cenas (reward aprēķinam).
        # Izpilde pie tam vienmēr pēc open (nav nākotnes datu noplūdes).
    ...
    def _execute_close(self, idx):
        ...
        Atiestata steps_since_close uz 0 - labvēlības periods sākas šeit.
        """
        ...
        self.entry_price       = 0.0
        self.current_trade     = None
        self.steps_since_close = 0   # labvēlības periods sākas no šī brīža


# 6. solis - Vektorizētā vide (VecEnv):
# N_ENVS paralēlās vides vienā pavedienā dod sākuma pozīciju daudzveidību un izlīdzina gradientus apmācības laikā.
...
class AttentionBlock(nn.Module):
    ...
    Kauzālā maska nodrošina, ka svece i redz tikai sveces 0..i - imitē tiešsaistes lēmumu bez nākotnes skatīšanas.
    """
...
class TransformerActorCritic(nn.Module):
    ...
    Lēmuma pieņemšanai tiek izmantots tikai pēdējais secības tokens (x[:, -1]) - tas nes visa loga uzkrāto kontekstu.

    Aktieris un Kritiķis - atsevišķi divslāņu MLP ar kopīgu enkodētāju.
    Aktieria svaru inicializācija ar mazu gain=0.01 samazina sākotnējo entropiju un paātrina konverģenci salīdzinājumā ar standarta inicializāciju.
    """
    def __init__(self):
        super().__init__()
...
# Buferis uzkrāj (obs, action, reward, done, log_prob, value) viena rollout laikā, tad compute_gae_and_flatten() aprēķina priekšrocības ar GAE metodi (λ=0.95)
# un atgriež izlīdzinātas priekšrocības un atgriešanās vērtības PPO atjauninājumam.

class RolloutBuffer:
    """
    Glabā vienu rollout visām N_ENVS vidēm.
    compute_gae_and_flatten() aprēķina GAE-priekšrocības un atgriež datus formā (T×E, ...) mini-paketes PPO atjauninājumam.
...
print(f"Labvēlības periods: {EXPERIMENT['GRACE_PERIOD_STEPS']} sveces  |  "
      f"Atvēršanas sods: {EXPERIMENT['OPEN_PENALTY']}  |  "
      f"Min darījums: {EXPERIMENT['MIN_TRADE_BARS']} sveces  |  "
      f"Īsa darījuma sods: {EXPERIMENT['SHORT_TRADE_PEN']}")
...
def compute_sharpe_realistic(trades_df, periods_per_year=252):
    """
    Sharpe koeficients, aprēķināts pēc atsevišķu darījumu atgriešanās (nevis pēc kapitāla līknes svecēm). Tiek gadskārtoti caur reālo tirdzniecības biežumu:
    trades_per_year = n_trades / duration_days × 365.

    Tas ir godīgāk nekā kapitāla līknes dalīšana ar √(BARS_PER_YEAR) kā bija iepriekšējās koda implementācijās:
    pie retas tirdzniecības sveces-bāzētais Sharpe stipri pārspīlē stratēģijas kvalitāti, vidējojot nulles gaidīšanas periodus kā nulles atgriešanos.
    """
...
def run_backtest(model, use_commission, use_slippage, label="", rng_seed=42):
    """
    Palaiž apmācīto modeli uz testa izlases deterministiskā režīmā (argmax pēc logitiem). Fiksēts sākums un seed nodrošina reproducējamību starp palaišanām.
    """
    model.eval()
