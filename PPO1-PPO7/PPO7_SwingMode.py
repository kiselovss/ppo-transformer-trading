...
SWING MODE REŽĪMS
─────────────────────
Šis eksperiments paplašina Realistic Mode ar divpusēju laika ierobežojumu pozīcijām.
Visi pamatparametri paliek identiski galvenajai versijai, bet tiek pievienoti divi jauni mehānismi:

  - Minimālais darījuma ilgums (MIN_TRADE_BARS = 72 sveces = 6h)
      slēgšana ātrāk - SHORT_TRADE_PEN sods
  - Maksimālais darījuma ilgums (MAX_TRADE_BARS = 288 sveces = 24h)
      turēšana ilgāk - LONG_TRADE_PEN sods katru soli

Aģents tiek stimulēts darboties "swing" diapazonā starp 6h un 24h.
Ārpus šī diapazona abās pusēs tiek piemērots vienāds sods (0.000898),
kas ir salīdzināms ar OPEN_PENALTY un SHORT_TRADE_PEN.

Mērķis: novērst gan haotisku skalping (pārāk īsas pozīcijas),
gan bezgalīgu zaudējošu pozīciju turēšanu (pārāk garas pozīcijas).

IZMANTOŠANA
───────────
Salīdzinu bektesta rezultātus ar Realistic Mode (pamatversiju):
  Ja vidējais darījuma ilgums pieaug, tad swing sods darbojas
  Ja Sharpe vai Calmar uzlabojas, tad laika ierobežojums palīdz kvalitātei
  Ja darījumu skaits krasi samazinās, tad MAX_TRADE_BARS iespējams par zemu
...
EXPERIMENT = {
    ...
    # Minimālais sveces skaits pozīcijā (identisks pamatversijai: 72 sveces = 6h).
    "MIN_TRADE_BARS"     : 72,

    # Sods par pārāk īsu pozīciju — identisks pamatversijai.
    "SHORT_TRADE_PEN"    : 0.000898,

    # Maksimālais sveces skaits pozīcijā pirms soda.
    # 288 sveces = 24h pie 5min svecēm. Ja turēšana ilgāk — aktivizējas LONG_TRADE_PEN.
    # Mērķis: novērst bezgalīgu pozīciju turēšanu zaudējumos (skat. swing trading loģiku).
    "MAX_TRADE_BARS"     : 288,

    # Sods par pārāk ilgu pozīciju (spoguļattēls SHORT_TRADE_PEN).
    # Tiek piemērots katru soli pēc MAX_TRADE_BARS robežas pārsniegšanas.
    "LONG_TRADE_PEN"     : 0.000898,
}
...
def compute_reward(action, position, floating_eq, prev_balance,
                   ...
                   cfg=EXPERIMENT):
    """
    Atlīdzības politika (Swing Mode):
    1. Nederīga darbība                       = INVALID_ACTION_PEN
    2. HOLD bez pozīcijas labvēlības periodā  = 0.0
    ...
    7. Turēšana ilgāk par MAX_TRADE_BARS      = reward - LONG_TRADE_PEN (katru soli)
    8. Visas vērtības tiek apgrieztas ±REWARD_CLIP

    Swing Mode papildinājums salīdzinājumā ar pamatversiju:
    7. punkts — divpusējs laika ierobežojums pozīcijām.
    """
    ...
    # Anti-skalping sods: pārāk īsa pozīcija
    if action == 3 and bars_in_trade < cfg["MIN_TRADE_BARS"]:
        reward -= cfg["SHORT_TRADE_PEN"]

    # Swing sods: pārāk ilga pozīcija (katru soli pēc MAX_TRADE_BARS)
    # Stimulē aģentu slēgt pozīcijas savlaicīgi, nevis turēt bezgalīgi.
    if position != 0 and bars_in_trade > cfg["MAX_TRADE_BARS"]:
        reward -= cfg["LONG_TRADE_PEN"]
