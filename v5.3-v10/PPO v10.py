...
VERSION = "Realistic Mode, OP - 0.000898 (Main)"
SYMBOL  = "BTC"
...
EXPERIMENT = {
    "name"               : "OP - 0.000898 (Main)",
    "SEED"               : SEED,
    "TOTAL_STEPS"        : TOTAL_STEPS,

    "HOLD_PENALTY_NO_POS": -0.003,   # sods par HOLD bez pozīcijas (ārpus labvēlības perioda)
    "INVALID_ACTION_PEN" : -0.05,    # sods par nederīgu darbību (piemēram, CLOSE bez pozīcijas)
    "REWARD_CLIP"        : 0.3,      # reward ierobežojums +-0.3
    "DD_COEF"            : 0.0,      # drawdown soda koeficients (atslēgts)

    # Sveces skaits pēc pozīcijas slēgšanas, kuru laikā HOLD netiek sodīts.
    # Dod aģentam laiku padomāt pirms nākamā ieejas tirgū.
    "GRACE_PERIOD_STEPS" : 12,

    # Sods par darījuma atvēršanu. Ierobežo pārmērīgu tirdzniecību.
    # GPU vērtības: 0.004 (aktīvais) / 0.0064 (pamatrežīms) / 0.007 (konservatīvais).
    # CPU vērtības: 0.0001-0.004 (sīkāk aprakstīts faila sākumā).
    "OPEN_PENALTY"       : 0.000898,

    # Minimālais sveces skaits pozīcijā. Ja noslēdz ātrāk - aktivizējas
    # SHORT_TRADE_PEN. Tika atlasīts kopā ar OPEN_PENALTY.
    # 12, tad 48, tad 96, pašreizējais: 72
    "MIN_TRADE_BARS"     : 72,

    # Sods par "skalping" (pozīcijas slēgšanu ātrāk par MIN_TRADE_BARS).
    # Jābūt salīdzināmam ar OPEN_PENALTY, citādi aģents ignorē ierobežojumu.
